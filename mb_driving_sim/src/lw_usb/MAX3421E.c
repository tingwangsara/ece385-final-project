...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE WriteBuffer[2] = {reg + 2, val};
	BYTE ReadBuffer[2] = {0, 0};
	int retCode = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, 2);
	if (retCode != XST_SUCCESS){
		print("SPI device failed to transfer\n");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int retCode = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, nbytes+1);
	if (retCode != XST_SUCCESS){
			print("SPI multi-byte error\n");
		}
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	return (data + nbytes);
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//return val
	XSpi_SetSlaveSelect(&SpiInstance, 0x01);
	BYTE WriteBuffer[2] = {reg, 0};
	BYTE ReadBuffer[2];
	int retCode = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, 2);
	if (retCode != XST_SUCCESS){
				print("SPI Single host register read error\n");
			}
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	return (ReadBuffer[1]); //val
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int retCode = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, nbytes+1);
	if (retCode != XST_SUCCESS){
					print("SPI multiple-bytes register read error\n");
				}
